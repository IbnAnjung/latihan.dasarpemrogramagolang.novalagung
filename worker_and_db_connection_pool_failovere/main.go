...
const dbConnString = "root:secret@/novalagung"
...
func dispatchWorker(db *sql.DB, jobs <-chan []interface{}, wg *sync.WaitGroup) {
	for workerIndex := 0; workerIndex <= totalWorker; workerIndex++ {
		go func(workerIndex int, db *sql.DB, jobs <-chan []interface{}, we *sync.WaitGroup) {
			counter := 0
...
func main() {
	...
	fmt.Print("done in", int(math.Ceil(duration.Seconds())), "seconds")
}
